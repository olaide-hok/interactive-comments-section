...
interface ReplyCardReplyFieldProps {
    commentId: number;
    replyId: number;
    setShowReplyField: React.Dispatch<React.SetStateAction<boolean>>;
}

const ReplyCardReplyField = ({
    commentId,
    replyId,
    setShowReplyField,
}: ReplyCardReplyFieldProps) => {
    const {replyReply} = useCommentsStore();
    ...
    const handleReplyComment = () => {
        ...
        replyReply(commentId, replyId, replyText.trim());
        setReplyText('');
        setShowReplyField(false);
    };

    return (
        <div className="flex flex-col md:flex-row items-start justify-center gap-y-(--sp-200) gap-x-(--sp-200) bg-(--clr-white) p-(--sp-200) md:p-(--sp-300) rounded-lg w-[20.3rem] md:w-[40.1rem]">
            <Image
...
export default ReplyCardReplyField;
